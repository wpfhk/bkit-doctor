...
### `skill` — 生成 SKILL.md 自动化规则

生成 `SKILL.md` 文件，教会 Claude Code 如何自动使用 bkit-doctor。

```bash
bkit-doctor skill                     # 在当前目录生成 SKILL.md
bkit-doctor skill --append-claude     # 同时将规则追加到现有 CLAUDE.md
```

| 规则 | 行为 |
|------|------|
| RULE 1: PROACTIVE DOCUMENTATION | 编码前自动运行 `pdca-plan` |
| RULE 2: STATE SYNC | 实现前检查 `pdca-list` 状态 |
| RULE 3: PIPELINE | 编码后自动运行 `pdca-do`, `pdca-check`, `pdca-report` |

---

### `setup` — 交互式项目设置向导

一键完成诊断、生成 `CLAUDE.md`、生成 `SKILL.md`、添加 npm 快捷脚本。
...
1. 运行 `check` + `fix`
2. 生成 `CLAUDE.md`（已存在时备份为 `CLAUDE_{日期}_backup.md` 后重新生成）
3. 生成 `SKILL.md`
4. 向 `package.json` 添加 `bkit:check`, `bkit:fix`, `bkit:setup` 脚本
