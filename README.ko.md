...
### `skill` — SKILL.md 자동화 규칙 생성

Claude Code가 bkit-doctor를 자동으로 사용하는 방법을 담은 `SKILL.md` 파일을 생성합니다.

```bash
bkit-doctor skill                     # 현재 디렉터리에 SKILL.md 생성
bkit-doctor skill --append-claude     # 기존 CLAUDE.md에도 규칙 추가
```
...
| RULE 3: PIPELINE | 코딩 후 `pdca-do`, `pdca-check`, `pdca-report` 자동 실행 |

---

### `setup` — 인터랙티브 프로젝트 설정 위저드

진단, `CLAUDE.md` 생성, `SKILL.md` 생성, npm 단축 스크립트 추가를 한 번에 처리합니다.
...
1. `check` + `fix` 실행
2. `CLAUDE.md` 생성 (기존 파일은 `CLAUDE_{날짜}_backup.md` 백업 후 재생성)
3. `SKILL.md` 생성
4. `package.json`에 `bkit:check`, `bkit:fix`, `bkit:setup` 스크립트 추가
